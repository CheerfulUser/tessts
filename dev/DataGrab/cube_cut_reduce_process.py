...
message = f'\nCONFIRMING PROCESS\n\
    ...
    - CCD: {ccd}\n\
    \n\
    - Download Number: {download_number}\n\
    - Cut: {cut}\n\
    - n: {n}\n\
    \n\
    - Batch Time 1: {time1}:00\n\
    - Batch Time 2: {time2}:00\n\
    - CPUs: {cpus}\n\
    - Memory/CPU: {mem}G\n\
CONTINUE? [y/n] :\n '
...
if go:
    ...
    while not finished:
        if os.path.exists(f'{data_path}/Sector{sector}/Cam{cam}Ccd{ccd}/'):
            folder_length = len(glob(f'{data_path}/Sector{sector}/Cam{cam}Ccd{ccd}/*ffic.fits'))
            sleep(60)
            new_length = len(glob(f'{data_path}/Sector{sector}/Cam{cam}Ccd{ccd}/*ffic.fits'))
...
n = {n}\n\
\n\
grabber = DataGrab(sector=sector,path=data_path,verbose=1)\n\
